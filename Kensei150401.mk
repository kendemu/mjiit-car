...
#
#  $RCSfile: Kensei150401.mk $ $Revision: 2.76 $ $Date: 2014/02/25 16:00:51MEZ $
...
MAKEFILE                       = Kensei150401.mk
BUILDARGS                      =  GENERATE_REPORT=0 EXTMODE_STATIC_ALLOC=0 TMW_EXTMODE_TESTING=0 EXTMODE_STATIC_ALLOC_SIZE=1000000 EXTMODE_TRANSPORT=0 MAT_FILE=0
MATLAB_ROOT                    = C:\Program Files (x86)\MATLAB\R2012b
MATLAB_BIN                     = C:\Program Files (x86)\MATLAB\R2012b\bin
MATLAB_ROOT_ALT                = C:\PROGRA~2\MATLAB\R2012b
MATLAB_BIN_ALT                 = C:\PROGRA~2\MATLAB\R2012b\bin
RELEASE_VERSION                = R2012b
RELEASE_VERSION               := $(RELEASE_VERSION,S'\.'_',W_,LC,S'^r'')
COMPUTER                       = PCWIN
MODEL                          = Kensei150401
MODULES                        = Kensei150401_data.c Kensei150401_trc_ptr.c rtGetInf.c rtGetNaN.c rt_nonfinite.c 
S_FUNCTIONS                    = 
S_FUNCTIONS_LIB                = 
SOLVER                         = 
NUMST                          = 3
...
$(_TMP_OPTION_NAME)              = 192.168.140.1
$(_TMP_OPTION_NAME)             := $($(_TMP_OPTION_NAME),'=,"=)
%if    '$($(_TMP_OPTION_NAME))' != ''
% if   '$($(_TMP_OPTION_NAME))' == 'OPTION_DISABLED'
%   undef $(_TMP_OPTION_NAME)
...
TRZGEN      = "$(DSPACE_RTT_TOOLS)\variablemanager.exe"
...
%ifdef PLATFORM
%  set CMDLOAD_OPT  = -p $(PLATFORM)
%endif
%ifdef NCLIENT
%  set CMDLOAD_OPT  = -rnc $(NCLIENT) -t DS$(BOARD_TYPE)
%endif
...
XCP = "$(DSPACE_ROOT)\exe\XcpGen1401.exe"
...
%  set TRCFILE = $(WORKDIR)$(MODEL).trc
%  set TRZFILE = $(WORKDIR)$(MODEL).trz
%else
...
RTWCUST_INCLUDES = \
	$(START_DIR)\Kensei150401_rti1401 \
	$(START_DIR) \
...
TMP_PATH_CC += $(MATLAB_ROOT)\rtw\c\src
TMP_PATH_CC += C:\PROGRA~2\DSPACE~2\MATLAB\RTICAN\TLC
...
start_check :
	...
	%endif
	%if !(exists("$(TRZGEN,"=)"))
	  %echo ERROR: Cannot find $(TRZGEN,F) tool. Possible reasons are:
	  %echo        - Environment variable DSPACE_RTT_TOOLS not defined.
	  %echo        - Environment variable DSPACE_RTT_TOOLS points to wrong directory.
	  %do make_abort
	%endif	
	%if !(exists("$(XCP,"=)"))
	  %echo ERROR: Cannot find $(XCP,F) tool. Possible reasons are:
	  %echo        - RTLib$(BOARD_TYPE) not installed.
	  %echo        - Environment variable DSPACE_ROOT points to wrong directory.
	  %do make_abort
	%endif
...
make_init :
	%ifdef _IS_ROOT_MODEL
	  %do delete_file FILE_LIST="$(PROGRAM,$(WS_PACK_TARGET_ARGS)) $(MAPFILE,$(WS_PACK_TARGET_ARGS)) $(TRZFILE,$(WS_PACK_TARGET_ARGS)) $(MODEL).rtimakefinish"
	%endif
	%ifdef COMPALL
	  %if '$(COMPALL, UC)' == 'ON'
...
link_main :
	...
	%do postlink_generate_trz
	%do postlink_generate_xcp
...
postlink_generate_trz:
    #
    # Generate trz file for real-time testing
    #
    %if "$(ENABLE_REAL_TIME_TESTING,UC)" == "ON"
        # -g            generate new TRZ file from existing TRC and MAP file
        $(EXEC_SHELL) $(TRZGEN) -g "$(TRCFILE,A)"
        %if $(status)
          %set _STATUS = $(status)
          %echo GENERATING TRZ-FILE FOR REAL-TIME TESTING $(TRZFILE,F) FAILED ($(_STATUS))
          %do make_abort
        %endif
    %endif

postlink_generate_xcp:
    #
    # Convert dSPACE real-time application into load-file format used by XCP loader tools
    #
    %if exists($(PROGRAM))
        # -q               quiet mode :  no info messages
        # -srec <fileName> generate output file in Motorola S-record format
        # -ihex <fileName> generate output file in Intel I32 hex format
        $(EXEC_SHELL) $(XCP) $(PROGRAM) -q
        %if $(status)
          %set _STATUS = $(status)
          %echo CONVERTING $(PROGRAM,F,UC) for XCP FAILED ($(_STATUS))
          %do make_abort
        %endif
    %endif
...
download :
	...
	%if ('$(LOAD_OPT)' == 'ON') || ('$(LOAD_OPT)' == 'FLASH')
	  ...
	  %set SLAVE_OPT =
	  %ifdef SLAVE
	    %set SLAVE_OPT = /l $(SLAVE)
	    %set LOAD_MSG := $(LOAD_MSG)$(NEWLINE)  SLAVE:      $(SLAVE)
	  %endif
	  %echo
	  %echo LOADING APPLICATION "$(SDFFILE,F)" $(LOAD_MSG)
	  --$(CMDLOADER) -ol 2 $(FLASH_OPT) $(CMDLOAD_OPT) $(SDFFILE) $(SLAVE_OPT)
	  %if $(status)
